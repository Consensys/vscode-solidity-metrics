  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      ...
      async (clickedFileUri, selectedFiles) => {
        ...
        if (path.basename(clickedFileUri.fsPath) === settings.extensionConfig().scopefile.name) {
          return await computeMetricsFromScopeFile(clickedFileUri.fsPath,selectedWorkspace,webView); //scope file
        }
              ...
              selectedFiles
                .map((x) =>
                  x.fsPath.endsWith('.sol')
                    ...
                    : x.fsPath.replace(metrics.basePath, '') + '/**/truffle*.js'
                )
                .join(',') +
              '}';
